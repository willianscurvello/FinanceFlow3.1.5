...
export function ExpensesPage() {
  ...
  const [selectedEmployee, setSelectedEmployee] = useState<string>(''); // Estado para o empregado selecionado
  const [filteredExpenses, setFilteredExpenses] = useState<Expense[]>([]); // Estado para armazenar as despesas filtradas
  ...
  useEffect(() => {
    // Filtrar despesas sempre que a lista de despesas ou os filtros mudarem
    const applyFilters = () => {
      let filtered = expenses;

      if (selectedCompany) {
        filtered = filtered.filter(expense => expense.company_id === selectedCompany);
      }

      if (selectedEmployee) {
        filtered = filtered.filter(expense => expense.employee_id === selectedEmployee);
      }
    ...
    applyFilters();
  }, [expenses, selectedCompany, selectedEmployee]);
  ...
  const fetchExpenses = async () => {
    ...
    if (error) {
      ...
      setFilteredExpenses(expensesWithNames); // Inicializar despesas filtradas
    }
  };
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-4">
          ...
          <select
            ...
            onChange={(e) => setSelectedEmployee(e.target.value)} // Atualizar o empregado selecionado
            className="rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
          >
      ...
      {isPopupOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-lg mx-4">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{newExpense.id ? 'Editar Gasto' : 'Adicionar Gasto'}</h2>
